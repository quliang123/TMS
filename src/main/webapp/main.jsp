    ...
    <table style="padding: 5px;" width="100%">
        <tr>
            ...
            <td valign="bottom" align="right" width="50%">
                <font size="3">
                    &nbsp;&nbsp;<strong>欢迎：</strong>${act_id_user.rev_}(${act_id_user.firstName }${act_id_user.lastName })【${act_id_user.act_id_group.name_}】</font>
            </td>
        </tr>
    </table>
</div>
            ...
            <div title="${item.name}" data-options="selected:true,iconCls:'${item.icon}'" style="padding: 10px">
                <c:forEach items="${item.children}" var="child">
                    <a href="javascript:openTab('${child.name}','${child.url}','${child.icon}')"
                       class="easyui-linkbutton" data-options="plain:true,iconCls:'icon-user'" style="width: 150px">${child.name}</a>
                </c:forEach>
            </div>
        </c:forEach>
            ...
            <tr>
                ...
                <td>
                    <input type="text" id="userId" name="userId" readonly="readonly"
                           value="${currentMemberShip.user.id }" style="width: 200px"/>
                </td>
            </tr>
            <tr>
                <td>原密码：</td>
